...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_1 = nh.subscribe("kinect_skeleton", 1000,
  ...
  while(ros::ok())
  {
    ...
    if (n_kinect_points > 0)
    {
      ...
      // Move the head (in the joint space)
      // ------------------------------------------------------------------
      ...
      if (qheadyaw > 2.0)    qheadyaw = 2;
      if (qheadyaw < -2.0)   qheadyaw = -2.0;
      if (qheadpitch > 0.5)  qheadpitch = 0.5;
      if (qheadpitch < -0.6) qheadpitch = -0.6;
      // Avoid small increments (noise) for the head pitch
      if (fabs(qheadpitch-qheadpitch_prev) > 0.02) {
        qdes[1] = qheadpitch;
        qheadpitch_prev = qheadpitch;
      }
      else
        qdes[1] = qheadpitch_prev;
      // Avoid small increments (noise) for the head yaw
      if (fabs(qheadyaw-qheadyaw_prev) > 0.02) {
        qdes[0] = qheadyaw;
        qheadyaw_prev = qheadyaw;
      }
      else
        qdes[0] = qheadyaw_prev;
